...
OSMDI_BOTNAME = os.getenv(
    "OSMDI_BOTNAME", "osmdi-cli-bot/" + _REFVER
)
_OSMDI_BOT_CONTACT_DEFAULT = (
    "https://github.com/fititnt/osmdi-py; generic@example.org"
)
OSMDI_BOT_CONTACT = os.getenv(
    "OSMDI_BOT_CONTACT", _OSMDI_BOT_CONTACT_DEFAULT
)
OSMDI_LIB = f"osmdi/{_REFVER}"

_USER_AGENT_MERGED = (
    f"{OSMDI_BOTNAME} ({OSMDI_BOT_CONTACT}) {OSMDI_LIB}"
)
...
OSMDI_BOT_CUSTOM_DELAY = int(
    os.getenv("OSMDI_BOT_CUSTOM_DELAY", DELAY_NONGENERIC_CONTACT)
)
...
_JSONLD_CONTEXT = (
    # "https://raw.githubusercontent.com/fititnt/osmdi-py/main/context.jsonld"
    "https://wtxt.etica.ai/context.jsonld"
)
_JSONSCHEMA = (
    # "https://raw.githubusercontent.com/fititnt/osmdi-py/main/schema.json"
...
def delay_consecutive_request():
    ...
    if OSMDI_BOT_CUSTOM_DELAY > DELAY_NONGENERIC_CONTACT:
        return OSMDI_BOT_CUSTOM_DELAY

    if OSMDI_BOT_CONTACT == _OSMDI_BOT_CONTACT_DEFAULT:
...
def osmdi_all(
    ...
    data = {
        ...
        # "@context": "https://raw.githubusercontent.com/fititnt/osmdi-py/main/context.jsonld",
        "@context": _JSONLD_CONTEXT,
        # "$schema": "https://raw.githubusercontent.com/fititnt/osmdi-py/main/schema.json",
        "$schema": _JSONSCHEMA,
    ...
    if template_keys is not None and len(template_keys) > 0:
        for item in template_keys:
            result = osmdi_from_infobox(wikitext, item)
            if result:
    ...
    if syntaxhighlight_langs is not None and len(syntaxhighlight_langs) > 0:
        for item in syntaxhighlight_langs:
            results = osmdi_from_syntaxhighlight(wikitext, item)
            # results = osmdi_from_syntaxhighlight(wikitext)
            if results:
...
def osmdi_from_infobox(
    wikitext: str, template_key: str, id_from: List[str] = None
):
    """osmdi_from_infobox Parse typical Infobox
...
def osmdi_from_syntaxhighlight(
    wikitext: str, lang: str = None, has_text: str = None, match_regex: str = None
) -> List[tuple]:
    """osmdi_get_syntaxhighlight _summary_
...
def osmdi_meta(wikitext: str) -> dict:
    return {}


def osmdi_meta_from_api(wikiapi_meta: dict) -> dict:
    meta = {}
...
def osmdi_raw(wikitext: str) -> dict:
    return wikitext
...
class WikitextAsData:
    ...
    def output_jsonld(self):
        ...
        # if not self.errors:
        if self.is_success():
            # return osmdi_all(self.wikitext, _next_release=True)
